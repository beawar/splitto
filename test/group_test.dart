...
import 'package:splitto/src/group.dart';
...
void main() {
  ...
  test('should throw if two members have the same name', () {
    ...
    expect(() => Group(members), throwsArgumentError);
  });

  test('should add a payment for a member with all members as borrowers by default (payer included)', () {
    final payer = Member('payer');
    final borrowers = [Member('borrower 1'), Member('borrower 2')];
    final members = [...borrowers, payer];
    final group = Group([...borrowers, payer]);
  ...
  test('total amout should be the sum of the payments', () {
    ...
    final group = Group([member1, member2, member3]);
  ...
  test('debit of a member should be the sum of the quota of all payments for which that member is borrower and not payer', () {
    final member1 = Member('member 1');
    final member2 = Member('member 2');
    final member3 = Member('member 3');
    final group = Group([member1, member2, member3]);
  ...
  test('credit of a member should be the sum of payments for which that member is the payer without its quotas', () {
    final member1 = Member('member 1');
    final member2 = Member('member 2');
    final member3 = Member('member 3');
    final group = Group([member1, member2, member3]);
